      ...
      <div class="mx-8 text-white">
        ...
        >
          ...
          <div class="flex gap-2 flex-1 justify-end">
            <p>H: {{ Math.round(day.temp.max) }}&deg;<sup>c</sup></p>
            <p>L: {{ Math.round(day.temp.min) }}&deg;<sup>c</sup></p>
          </div>
        </div>
      </div>
    </div>
...
const getWeatherData = async () => {
  try {
    const weatherData = await axios.get(
      `https://api.openweathermap.org/data/2.5/onecall?lat=${route.query.lat}&lon=${route.query.lon}&appid=${openweathermapAPIKey}&units=metric`
    );
